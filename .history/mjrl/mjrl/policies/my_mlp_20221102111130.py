...
class MLP:
    def __init__(self, env_spec,
        ...
        # Policy network
        # ------------------------
        self.models = [FCNetwork(self.n, self.m, hidden_sizes) for i in range(anchor_dims)]
        # make weights small
        for model in self.models :
            for param in list(model.parameters())[-2:]:  # only last layer
        ...
        # Old Policy network
        # ------------------------
        self.old_model = [FCNetwork(self.n, self.m, hidden_sizes) for i in range(anchor_dims)]
