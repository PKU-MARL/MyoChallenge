...
class MyMLP:
    def __init__(self, env_spec,
        ...
        for model in self.models :
            ...
            self.old_params += model.old_params
    ...
    def log_likelihood(self, observations, actions, model=None, log_std=None):
        model = self.get_model(observations)
        return model.log_likelihood(observations, actions, model, log_std)

    def old_dist_info(self, observations, actions):
        model = self.get_model(observations)
        return model.old_dist_info(observations, actions)

    def new_dist_info(self, observations, actions):
        model = self.get_model(observations)
        return model.new_dist_info(observations, actions)
